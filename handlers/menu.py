...
@router.message(F.text == language_message("buttons_meme_menu", F)[-1])
...
@router.message(F.text == language_message("buttons_start_menu_private", F)[-1])
async def other_menu(message: Message):
    # Меняем клавиатуру на другую и выводим имя этой клавиатуры
    await message.answer(language_message("open_other_menu", message),
                         reply_markup=create_reply_keyboard(language_message("buttons_other_menu"), add_last=1))


@router.message(F.text == language_message("buttons_other_menu", F)[0])
