...
def sqliteStart():
    ...
    base = sq.connect('database/data/db/data.db')  # Подключаем файл с базой данных
    cur = base.cursor()
...
async def get_user_data_from_database(message=None, id_user=None):
    ...
    if user_info:
        ...
        # [7] - Язык пользователя
        string = str(language_data_json("profile_data")) \
            .format(user_info[0], user_info[1], user_info[3], user_info[5],
                    language_data_json("levels_users")[user_info[6]], user_info[7])
        ...
        await message.bot.send_message(id_user, language_data_json("edit_profile"))
...
async def edit_user_data_in_database(commands, edited_information, message, id_user=None):
    ...
    if not get_user_info_from_database(id_user=id_user):
        ...
        await message.answer(language_data_json("error_edit_data_not_reg"))
    ...
    base.commit()
    await message.answer(language_data_json("edited_data_user"),
                         reply_markup=create_reply_keyboard(language_data_json("buttons_start_menu_private")))
    logger.info(f"Пользователь изменил данные профиля: '{commands}' на '{edited_information}'", message=message,
             color_consol="yellow")
